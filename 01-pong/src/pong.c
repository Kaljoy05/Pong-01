...
void mover_paleta_ia(struct Pong* pong, double dt) {
    float centro_paleta = pong->player2.y + PADDLE_HEIGHT / 2;
    float centro_pelota = pong->ball.y + pong->ball.height / 2;
    float diferencia = centro_pelota - centro_paleta;

    if(pong->ball.x > (TABLE_WIDTH/2 + TABLE_WIDTH/6 ) ) {
        if (fabs(diferencia) > PADDLE_SPEED * dt) {
            diferencia = (diferencia > 0) ? PADDLE_SPEED * dt : -PADDLE_SPEED * dt;
        }
    
        pong->player2.y += diferencia;
    
        if (pong->player2.y < 0) {
            pong->player2.y = 0;
        }
        if (pong->player2.y + PADDLE_HEIGHT > TABLE_HEIGHT) {
            pong->player2.y = TABLE_HEIGHT - PADDLE_HEIGHT;
        }
    }
}
...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY)
    {
        ...
        mover_paleta_ia(pong, dt);  // La IA ahora mueve player2
